...
const KIND_HELP: Record<OrphanKind, string> = {
  sitemap:
    "Listed in a sitemap and linked from nowhere. Published to search engines, " +
    "hidden from visitors — the finding worth acting on.",
  cms:
    "Present in the CMS database only. Never published in a sitemap and never " +
    "linked, so it may be a draft or a retired record.",
  unlinked:
    "Reached during the crawl but holding no inbound link of its own. Usually a " +
    "page whose only linker was never fetched — check before reporting it.",
};
 ...
 *
 * A result stored before the profile carried these flags arrives with the field
 * absent, which is indistinguishable from "found by no path at all" — so every
 * orphan in an old crawl would be labelled `Crawl only` and the split would read
 * as a finding about the site instead of a gap in the data. A view that quietly
 * mislabels a thousand pages is worse than one that declines to split them, so
 * the control is withdrawn and the reason is stated.
 */
function hasProvenance(pages: readonly FullPageIntelligenceProfile[]): boolean {
  return pages.some((item) => {
    ...
    return Boolean(sources?.sitemap || sources?.dom_link || sources?.cms_api);
  });
}
...
export function OrphanTable({
  ...
  const counts = useMemo(() => {
    const tally: Record<OrphanKind, number> = { sitemap: 0, cms: 0, unlinked: 0 };
  ...
  const rows = useMemo(() => {
    ...
    return pages
      ...
      .filter((page) => needle === "" || page.url.toLowerCase().includes(needle));
  }, [pages, filter, query]);
  ...
  const exportCsv = (): void => {
    const csv = toCsv(
      ["url", "orphan_kind", "page_type", "hierarchy_level", "sitemap_source", "topical_category"],
      rows.map((page) => [
        page.url,
        orphanKind(page),
        page.primary_page_type,
        page.hierarchy_level,
        page.sitemap_source ?? null,
        page.topical_category,
      ]),
    );
    downloadCsv(`${hostSlug(baseUrl)}-orphans${filter === "all" ? "" : `-${filter}`}.csv`, csv);
  };

  // Withdrawn rather than blanked when the crawl predates the flags: a column of
  // identical grey tags is a claim about the site that the data cannot make.
    ...
    ? [
        {
          ...
          key: "kind",
          width: 120,
          filters: (["sitemap", "cms", "unlinked"] as const).map((kind) => ({
          ...
          render: (_, page) => {
            ...
            return (
              <Tooltip title={KIND_HELP[kind]}>
                <Tag color={KIND_COLOUR[kind]}>{KIND_LABEL[kind]}</Tag>
              </Tooltip>
            );
          },
        },
    ...
    <div className="au-drill">
      {!known && (
        <p className="au-stale">
          This crawl ran before the engine recorded which path found each URL, so these
          orphans cannot be split into published and unpublished. Re-crawl the site to get
          that breakdown — a reparse cannot recover it, because the information was never
          stored.
        </p>
      )}
      ...
      <div className="au-tools">
        {known && (
          <Segmented
            ...
            options={[
              { label: `All ${pages.length.toLocaleString()}`, value: "all" },
              { label: `Sitemap ${counts.sitemap.toLocaleString()}`, value: "sitemap" },
              { label: `CMS only ${counts.cms.toLocaleString()}`, value: "cms" },
              { label: `Crawl only ${counts.unlinked.toLocaleString()}`, value: "unlinked" },
            ]}
          />
        )}
        <Input.Search
      ...
      <Table<FullPageIntelligenceProfile>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (source: string | null) => source ?? <span className="au-more">—</span>,
          },
        ]}
      />
    </div>
