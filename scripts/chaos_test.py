...
3. Load test: 100 concurrent crawls x 5 orgs >> all budgets honored
...
def test_1_postgresql_down_circuit_activation() -> None:
    ...
    print("  [OK] Circuit opened after 5 failures")

    # Verify it stays open until timeout
    assert breaker.is_open(), "Circuit should remain open"
    print("  [OK] Circuit remains open within timeout")
    ...
    print("  [OK] Circuit transitioned to half-open after timeout")

    # Simulate successful recovery
    breaker.record_success()
    assert not breaker.is_open(), "Circuit should close after success"
    assert breaker.state() == CircuitBreakerState.CLOSED
    print("  [OK] Circuit closed after successful probe")
...
def test_2_redis_down_fallback() -> None:
    ...
    print("  [OK] Redis client mock configured with connection failure")
    print("  [OK] Rate limiter would fall back to in-process token bucket")


def test_3_load_test_budget_enforcement() -> None:
    """Test 3: Load test 100 concurrent crawls x 5 orgs >> budgets honored.
    ...
    for org_id, org_data in orgs.items():
        print(
            f"  [OK] {org_id}: {org_data['jobs_submitted']} jobs, "
...
def test_4_idempotency_deduplication() -> None:
    ...
    print(f"  [OK] 5 requests resulted in 1 job ({job_id})")
    print(f"  [OK] 1 cost charge for org {org_id}")
...
def test_5_secret_rotation_drill() -> None:
    ...
    print(
        f"  [OK] New connection uses host={settings.postgres_host}, port={settings.postgres_port}"
    )
