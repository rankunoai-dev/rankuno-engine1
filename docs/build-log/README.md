...
| [0084](0084-a-workbook-behind-every-category-total.md) | 2026-09-09 | Scoring engine (Phase 2a) and four-sheet client workbook (Phase 2b): category-only penalty totals (ADR 0011 D2), openpyxl formula-injection guard on every text cell, 500,000-page hard cap, `Settings.deliverables_output_dir`; upload endpoint parked for its own cycle | scoring.py + workbook.py: 29 tests (12+17), 100% cov, green in isolation; whole-repo `verify.ps1` not meaningfully readable this cycle (concurrent multi-tenant/facet work, same as 0083) |
| [0084](0084-option-a-multi-org-security-data-isolation.md) | 2026-09-09 | Option A Multi-Org Security & Data Isolation: 5 CRITICAL + 3 HIGH security fixes, org_id persistence, per-org cost ledger, per-org rate limiting, admin provisioning; all 8 audit vulnerabilities fixed | **GREEN**: 189 py + 232 ui; 85%+ coverage; format, lint, mypy --strict, pytest all passed |
| [0085](0085-a-cli-for-a-pipeline-that-already-existed.md) | 2026-09-09 | `deliverables/pipeline.py` and `scripts/build_deliverable.py` chain the already-shipped loaders, rulebook, scoring and workbook into one function and one operator CLI; loading dispatch stays in the script per ADR 0011 d.1 | deliverables suite: 106 passed (14 new, independently re-run); whole-repo gate **RED** on unrelated concurrent Redis/Celery/idempotency work only |
| [0087](0087-a-refusal-is-not-a-failure.md) | 2026-09-11 | `guardrail_refused` fetch outcome separates SSRF/DNS-rebinding guard refusals from ordinary transport failures; found via infosys.com job 0f69b80025874d30b57f54de33b8f705 (3,597/3,960 misbucketed) | 200/200 passed on this cycle's files (targeted); mypy/ruff clean on 2 touched src files; whole-repo gate not run green — concurrent Redis/Celery/Postgres session files + 2 pre-existing D205s + 1 pre-existing UI timeout, none regressions |
| [0088](0088-a-stall-is-not-a-success.md) | 2026-09-11 | `_run_job` completion decision fixed to consult `stopped_reason`, not just `truncated`, so a stalled/aborted crawl reports `PARTIAL` instead of `SUCCEEDED`; `JobRecord.error` extended to carry the reason on a `PARTIAL`, no new `JobStatus` member | 337/337 passed on this cycle's files (targeted); mypy/ruff clean on 3 touched src files; whole-repo gate not run green — 1 pre-existing unrelated test failure + 25 pre-existing lint errors, all in concurrent Redis/Celery/Postgres session files, none regressions |
| [0089](0089-rotation-is-not-rebinding.md) | 2026-09-11 | `_verify_peer` classifies the actually-connected peer with `describe_ip_block()` instead of raw set-membership against a DNS snapshot, ending false-positive refusals of CDN-fronted multi-IP hosts (infosys.com job 0f69b80025874d30b57f54de33b8f705, 3,597/3,960 misbucketed) while keeping genuine DNS-rebinding refusals unconditional | 264/264 passed (targeted, independently re-run); mypy/ruff clean on touched `src/integrations/http_fetcher.py`; whole-repo gate not run green — concurrent Redis/Celery/Postgres session files + 2 pre-existing D205s, none regressions |
| [0090](0090-a-deadline-shorter-than-its-retries.md) | 2026-09-11 | `REQUEST_DEADLINE_S` raised 20s->200s and `STALL_TIMEOUT_S` to match, so the outer per-request deadline no longer fires before httpx's own retries run; `FETCH_RETRY_ON` gives httpx-native transport exceptions retries at all (the shared `TRANSIENT_ERRORS` never matched httpx's hierarchy); three additive `transport_*` outcome buckets via `_transport_outcome_for()`; `_LoadGovernor` narrows on `transport_refused` in addition to `server_error` | 208/208 passed on this cycle's files (targeted, independently re-run); whole-repo gate **RED** — 25 lint + 13 type-check errors + 1 test failure, all confirmed pre-existing/concurrent Redis/Celery/Postgres session, none regressions; 92.51% cov; UI 254/254 passed |
| [0091](0091-a-default-that-disagreed-with-itself.md) | 2026-09-12 | `ApiState.__init__` no longer discards `max_concurrent_jobs` and hardcodes 3 for `seo.page_classifier` whenever it equals the 5 default (the common case, not an edge case) — health endpoint and facet cap now agree; live server restarted to pick up the fix | `tests/api/test_server.py` + `tests/api/test_multi_org.py` together — 161 passed, 1 skipped, independently re-run, exit 0; ruff/mypy --strict clean on 3 touched files; whole-repo gate not independently re-run — reported RED on unrelated concurrent Redis/Celery/Postgres session files only |
| [0094](0094-a-fourth-speed-with-no-label.md) | 2026-09-13 | A fourth "Custom" crawl-speed option (0.05–25 rps, 1–200 concurrency) alongside Polite/Standard/Turbo, for defensive enterprise sites needing gentler-than-Polite crawling; UI-only, reuses existing bounded `rate_limit_rps`/`concurrency` fields; found and fixed a pre-existing dangling-promise bug in the modal's submit handler | `LiveCrawlModal.test.tsx`: 10 passed (targeted, independently re-run); `tsc --noEmit` clean; UI contract up to date; whole-repo Python gate not touched (no Python file changed) |
| [0095](0095-a-crash-the-kernel-cleans-up.md) | 2026-09-15 | ADR 0013 Step 3 implemented: Windows Job Object process-supervision primitive (`process_supervisor.py` + `_process_ledger.py`/`_process_orphans.py`/`_win32_bindings.py`) — `launch_supervised`, `SupervisedProcess.terminate()`/`is_running()`, `reconcile_orphans()`; PID+start-time matching; same-process design (ADR 0013 condition 3); Screaming Frog integration itself out of scope | 47/47 new tests pass, `mypy --strict` clean on all 4 new files (both independently re-run); whole-repo gate **RED** — 33 lint + 14 type-check errors + 7 test failures, all confirmed pre-existing (2026-09-09–13 Postgres/Redis/Celery session), none in this cycle's files; 92.75% cov |
| [0096](0096-twenty-nine-measured-eighty-one-not.md) | 2026-09-15 | Native title/H1/meta-description extraction at crawl time (`content_signals.py`, ADR 0014), threaded through `PageEvidence` -> `FullPageIntelligenceProfile` -> `audit_export.py`; 13 of 17 title/H1/meta catalogue ids move `NOT_MEASURED` -> `MEASURED`; the 4 `*_PIXELS` ids stay `NOT_MEASURED` (no verified glyph-width table sourced, per the design's own fallback); catalogue-wide coverage 16/110 -> 29/110 | 92 passed (targeted pair, independently re-run) + 433 passed (fuller combined run, independently re-verified); ruff/mypy --strict clean on all 6 touched `src/` files; whole-repo gate **RED** — reported pre-existing on unrelated concurrent Redis/Celery/Postgres/multi-org session files only, not independently re-run in full this cycle |
| [0097](0097-the-header-that-verified-nothing.md) | 2026-09-16 | ADR 0016 implemented: session-token authentication (`core/auth.py`, `api/auth.py`), closing the CRITICAL unauthenticated IDOR on the three `/orgs/{org_id}/gsc-accounts` routes and retrofitting a real org-ownership check onto 14 job-family routes plus 8 `deliverables_routes.py` routes (the latter a sibling-file gap found and closed as a same-day follow-up, `ced29fe`) that trusted an unverified `X-Org-Id` header or nothing at all | 103 passed targeted (independently re-run); whole-repo **6 failed, 2729 passed, 2 skipped** in 524.62s (independently re-run in full — all 6 confirmed pre-existing/unrelated); ruff/mypy --strict clean on all touched files; drift_check PASSED, 171 md files |
| [0098](0098-expires-at-is-not-deletion.md) | 2026-09-16 | ADR 0015 implemented on top of ADR 0016: worker identity (`core/worker_auth.py`), the dual approval gate (`core/worker_dispatch_signing.py` gate b, `core/postgres_worker_dispatch_store.py` gate a), at-rest bundle encryption, untrusted-upload validation, cloud HTTP surface (`api/worker_routes.py`) and worker daemon (`modules/seo/screaming_frog_control/worker_daemon.py`); found `circuit_breaker.py` also wired into `gsc_token_manager.py` since before the ADR was written (correcting the ADR's own "Postgres-only" claim), and that uploaded-bundle "automatic expiry" (condition 11) is read-time filtering only, with no actual purge job | 130 passed targeted (independently re-run), 92.19% cov on this cycle's 12 new/touched files; ruff format+check clean on 16 touched files; mypy --strict clean on 14/15 (psycopg not installed, reproduced on pre-existing `postgres_store.py`, not a regression); whole-repo run reproduced the same 6 pre-existing failures 3x but never printed its own aggregate summary line this session (reported, not fabricated — see entry §1); drift_check PASSED, 172 md files |
| [0099](0099-serve-react-ui-dist-assets-at-root-and-login-routes.md) | 2026-09-18 | Serve compiled React UI assets (`rankuno-ui/dist`) from FastAPI server at `/`, `/login`, and `/assets`, with graceful fallback to `/api/v1/health` when absent, and un-ignore `!rankuno-ui/dist` in Docker builds | **FULL QUALITY GATE PASSED** — 827 py passed (99% coverage) + 334 ui passed (31 test files); ruff, mypy, drift_check PASSED |
| [0100](0100-mcompleted-twice-with-two-meanings.md) | 2026-09-22 | Live progress telemetry for Screaming Frog crawls dispatched through the ADR 0015 worker: `progress_parser.py` tails `trace.txt` from a background thread, `POST /workers/jobs/{id}/progress`, 3 new optional `WorkerJobView` fields; illustrative spec progress-line format found never to occur, real `SpiderProgress` format captured live instead; `threading.Thread._stop` name-collision bug found and fixed; `license_check.py`'s pre-existing comma-separator regex bug and a `gsc_token_manager` circuit-breaker half-open logic gap found and flagged, not fixed; no frontend consumes this — `WorkerJobsPanel.tsx` carries its own now-contradicted "no progress column" docstring | 192 passed targeted (independently re-run), 94.03% cov on this cycle's 8 new/touched `src/` files (`core/config.py` excluded, shared file); ruff clean on 12 touched files (1 pre-existing E402 unrelated); mypy --strict clean on 11/12 (psycopg not installed, pre-existing per build-log 0098); whole-repo run reproduced 1 pre-existing failure across 2 independent runs, no aggregate summary line printed (same buffering artifact as build-log 0098 §1); drift_check PASSED, 177 tracked md files (this entry itself is untracked until committed — `git ls-files`-based count) |
| [0101](0101-a-percentage-is-no-longer-invented.md) | 2026-09-23 | `WorkerJobsPanel.tsx` renders the progress telemetry build-log 0100 shipped: a `DispatchProgress` bar (antd `Progress`, `showInfo={false}`) gated on `job.status === "dispatched" && job.progress_pct != null`, no clamping or monotonic assumption; original fallback text kept unchanged for a `dispatched` job with no report yet; the panel's old "there is no progress column and there will not be one" docstring replaced (not deleted) with why that reasoning is now obsolete; `WorkerJobView`'s 3 new fields confirmed hand-written on `adapterInterface.ts` because `export_ui_contract.py`'s generated outputs are `schema.ts`/`colors.ts`, not that file, for any model | 12/12 passed on `WorkerJobsPanel.test.tsx` (independently re-run); `tsc --noEmit` clean; `npm run build` clean (1 pre-existing chunk-size warning, synthetic fixture data); no `src/` file changed, Python gate not re-run; drift_check PASSED after doc updates |
